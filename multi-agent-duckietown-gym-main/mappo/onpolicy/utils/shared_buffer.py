...
class SharedReplayBuffer(object):
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, adj_matrix, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.gso[self.step + 1] = adj_matrix.copy()
        ...
        if active_masks is not None:
            path = (self.episode_length + 1) - (self.step + 1)
            # self.active_masks[(self.step + 1):] = active_masks.copy()
            self.active_masks[(self.step + 1):] = np.zeros((path, self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        gso = self.gso[:-1].repeat(num_agents, axis=0).squeeze(1)
        ...
        agent_masks =  np.repeat([i for i in range(num_agents)], episode_length)
        ...
        for indices in sampler:
            ...
            # agent_masks_batch = np.expand_dims(agent_masks[indices],axis=1)
            agent_masks_batch = agent_masks[indices]
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  ...
                  adv_targ, available_actions_batch, gso_batch, agent_masks_batch
