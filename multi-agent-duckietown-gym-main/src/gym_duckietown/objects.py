...
class WorldObj:
    ...
    def __init__(self, obj, domain_rand: bool, safety_radius_mult: float):
        ...
        # These initial values are loaded from the .yaml (map file),
        # otherwise they are set elsewhere during program execution
        self.pos = obj["pos"]
...
class DuckiebotObj(WorldObj):
    ...
    def __init__(
        ...
        # These were previously defined as lists
        self.action = [0, 0]
        self.speed = None
        ...
        # Real robot
            ...
            self.k = rospy.get_param(f"/{self.name}/kinematics_node/k")
            self.gain = rospy.get_param(f"/{self.name}/kinematics_node/gain")
            self.trim = rospy.get_param(f"/{self.name}/kinematics_node/trim")
            self.radius = rospy.get_param(f"/{self.name}/kinematics_node/radius")
            self.wheel_dist = rospy.get_param(f"/{self.name}/kinematics_node/baseline") # 0.10
            self.robot_width = robot_width
            self.robot_length = robot_length

            self.rospub_wheelVel = rospy.Publisher(
                f'/{self.name}/wheels_driver_node/wheels_cmd',
                WheelsCmdStamped,
                queue_size=1)
    ...
    def send_wheelVel(self, action):
        wheelVel_msg = WheelsCmdStamped()
        wheelVel_msg.header = Header()
        wheelVel_msg.header.stamp = rospy.Time.now()
        wheelVel_msg.vel_left, wheelVel_msg.vel_right = action

        self.rospub_wheelVel.publish(wheelVel_msg)
