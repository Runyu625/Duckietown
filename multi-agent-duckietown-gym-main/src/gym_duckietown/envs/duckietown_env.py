...
class DuckietownEnv(Simulator):
    ...
    def __init__(self, gain=1.0, trim=0.0, radius=0.0318, k=27.0, limit=1.0, kinematics=True, **kwargs):
        ...
        for i in range(self.n_agents):
            act_spaces += [
                spaces.Box(
                    low=self.wheel_vel_min,
                    high=self.wheel_vel_max,
        ...
        self.action_space = spaces.Tuple(tuple(act_spaces))
    ...
    def step(self, action):
        ...
        if self.kinematics:
            if not self.mappo:
                angle = np.array(action)[:, 1]
                vel = np.array(action)[:, 0]
            else:
                angle = action[0][:, 1]
                vel = action[0][:, 0]
            ...
            if not self.mappo:
                u_r_limited = np.array(action)[:, 1]
                u_l_limited = np.array(action)[:, 0]
            else:
                u_r_limited = action[0][:, 1]
                u_l_limited = action[0][:, 0]
