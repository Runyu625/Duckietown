...
class RolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False, test=False):
        if self.args.replay_dir != '' and evaluate and episode_num == 0:  # prepare for save replay of evaluation
            self.env.close()
        o, u, r, s, avail_u, u_onehot, terminate, padded = [], [], [], [], [], [], [], []
        self.env.reset()
        obs = self.env.reset()
        state = np.array(self.env.get_full_obs()).reshape(-1)
        o.append(obs)
        s.append(state)
        terminated = False
        win_tag = False
        ...
        while not terminated and step < self.episode_limit:
            ...
            for agent_id in range(self.n_agents):
                avail_action = np.tile(1, (self.env.n_agents, self.env.action_size))[0]
            ...
            obs, reward, terminated, _ = self.env.step(actions)
            render = True
            if render:
                self.env.render()
            # if test:
        ...
        #     avail_actions.append(avail_action)
        # avail_u.append(avail_actions)
        avail_action = np.tile(1, (self.env.n_agents, self.env.action_size))
